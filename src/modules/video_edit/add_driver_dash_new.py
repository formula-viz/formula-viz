...
class DriverDash:
    ...
    def _add_driver_component_package(
        ...
        if self.config["render"]["is_shorts_output"]:
            ...
            color_y_offset = -700
            color_x_offset = 270 if not is_left else -270

            sectors_y_offset = -200
            sectors_x_offset = -100
        else:
            base_y = -500
            if is_left:
                base_x = -1550
        ...
        color_strip = self._add_color_strip_to_vse(
            ...
            channel=self.cur_channel,
            position_x=base_x + color_x_offset,
    ...
    def _add_driver_sector_times(
        ...
        def add_sector_underlines():
            for i in range(3):
                ...
                color.transform.scale_x = (
                    0.03 if self.config["render"]["is_shorts_output"] else 0.04
                )
        ...
        self._add_sector_times(
            driver, sector_package, text_size, sector_time_x_positions, base_y + 50
        )
